...
public:
    ...
    void afficherTempsCompetitions() const {
        std::cout << "\nTemps de compétition de " << nom << " :" << std::endl;
        for (double t : temps_competitions) {
            std::cout << "- " << t << " s" << std::endl;
        }
    }

    // === Emplacements de trésors ===
    ...
    void retirerEmplacementTresor(const std::string& lieu) {
        if (emplacements_tresors.erase(lieu)) {
            std::cout << "Trésor retiré à : " << lieu << std::endl;
        } else {
            std::cout << "Aucun trésor trouvé à retirer à : " << lieu << std::endl;
        }
    }
...
int main() {
    ...
    p1->ajouterEmplacementTresor("glacier nord");
    p1->ajouterEmplacementTresor("caverne secrète");
    p1->ajouterEmplacementTresor("glacier nord"); // duplicata non ajouté
    p1->afficherEmplacementsTresors();

    p1->retirerEmplacementTresor("caverne secrète");
    p1->afficherEmplacementsTresors();
